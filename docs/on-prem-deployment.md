...
**A second, quieter bug found one deploy later — the "uneventful" docs-only deploy wasn't actually clean.** `docker compose up -d api` left the container running the *previous* deploy's image, with no error, across two consecutive deploys — confirmed via `docker inspect --format .Created`, which showed the container object hadn't been recreated since its first manual start during the incident above. `docker images` confirmed the correct new tag genuinely had been pulled; Compose's own recreate-detection simply never fired on it. Root cause in Compose's internals not fully chased down. Fixed by no longer trusting that detection at all: `docker compose up -d --force-recreate api` in both `deploy.sh` and `deploy/rollback.sh`, plus a post-deploy assertion that the running container's actual image matches the intended tag, failing loudly otherwise. This resets the "one clean deploy" criterion below — the docs-only deploy looked clean by every check that ran, but was not, which is exactly the failure mode worth being honest about before adding more moving parts.

**When to add the second replica:** the plan's own stated criterion was "prove the single-replica pipeline works completely first" — that bar is now met, including a real incident and recovery, not just a quiet happy-path run. Recommendation: one more **uneventful** deploy (a trivial change, no active incident) to confirm today's two fixes hold up under normal conditions, then add replica 2. The remaining work is smaller than it looked before this session: `PORT` was already env-driven (`config.port`), host networking is already the model, and the Track 1.4a scheduler lease already makes concurrent instances safe. What's left is genuinely just: a second `api` service block in compose with its own `PORT`/`container_name`, the one-time nginx `upstream` edit (§16 Track 1.4b prerequisites), and `deploy.sh`/`rollback.sh` pulling+starting both.
