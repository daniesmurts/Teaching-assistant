...
| 1.7 ◆ | **`deployment_id` dimension** across admin queries + fleet overview page | §5.4. The bulk of the dashboard work, done while there is one deployment to be wrong about. **🟡 partial, shipped 2026-08-15** — new **Развёртывания** page (§5.4's "new" row) is live, purely additive; converting `AdminOverview`/`AdminUsage`/`AdminCapacity`/`AdminErrors` to actually read from the control-plane tables is intentionally deferred — see below. |
...
**Deliberately deferred, not forgotten:** reworking the four EXISTING admin pages (`AdminOverview` → fleet cards, `AdminUsage` → deployment-filtered, `AdminCapacity` → split by mode, `AdminErrors` → grouped by deployment+version) is real rework touching pages people already use daily, materially riskier than adding one new page — scoped as its own follow-up slice rather than folded in here, matching how 1.1/1.1b and 1.4a/1.4b were split earlier in this track.
