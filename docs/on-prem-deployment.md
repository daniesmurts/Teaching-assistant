...
3. ✅ **One replica** to start, chosen deliberately over two — proved the whole pipeline (image build → registry → pull → migrate → restart) with the fewest moving parts before adding the nginx `upstream` complexity a second replica needs. **Second replica shipped the same day**, once that pipeline had a real track record — see below, this section's own trade-off note (single-replica deploys losing PM2's zero-downtime `pm2 reload`) no longer applies.
...
**Second replica: ✅ shipped 2026-08-15 (same day).** The gate was three ordinary, independently-verified production deploys on the unchanged single-replica pipeline (`780be08`, `efdbdd5`, `9bb644e`) — confirmed satisfied by checking `git log` between the real fix and this work rather than assumed. What actually shipped:

- **`deploy/docker-compose.cloud.yml`** rewritten around a `x-api-common` YAML anchor (the compose-spec's own sanctioned "extension field" convention for exactly this — a top-level key prefixed `x-` that compose itself ignores except for anchor reuse) — `api` (PORT 3000) and `api2` (PORT 3001) share everything else: image, `network_mode: host`, env file, restart policy, the bind-mounted `uploads/` volume. The healthcheck was generalised from a hardcoded `127.0.0.1:3000` to reading `process.env.PORT` at runtime, so one definition serves both replicas via the anchor rather than needing two.
- **`deploy/nginx/gradeassist.conf`** gained an `upstream ispum_api { least_conn; server 127.0.0.1:3000; server 127.0.0.1:3001; }` block — `least_conn` over round-robin because AI-grading request duration varies wildly (a quick quiz call vs. a multi-minute ВКР map-reduce pass), so routing to whichever replica has fewer in-flight requests balances load better than blind alternation. This repo file is HTTP-only by convention (certbot edits the live 443 block directly on the VM and that never flows back to git) — the equivalent edit has to be applied to *both* server blocks on the actual VM by hand.
- **`deploy.sh` and `deploy/rollback.sh`** both do a genuine **rolling** restart, not simultaneous recreation — recreating both containers at once would reproduce the exact all-at-once outage a second replica exists to eliminate. `api2` is force-recreated first; the script polls Docker's own `.State.Health.Status` (not a single curl) for up to 120s before touching `api`, so nginx's `least_conn` upstream always has at least one live backend to route to. The image-match assertion from the earlier stdin-eating-bug fix now checks *both* containers, and the final `[8/8]` health check hits `:3000` and `:3001` directly rather than only through nginx — nginx would happily report healthy off one working replica while the other silently failed its own recreate.
- **Verification, not assumption**: the compose file's YAML anchor/merge syntax was actually parsed (via `js-yaml` with the YAML 1.1 schema that supports merge keys, since the default schema doesn't) to confirm it resolves to the intended per-service config, not just visually inspected. Both heredocs in `deploy.sh`/`rollback.sh` were rendered with placeholder values (substituting `cat` for the real `ssh` call) and `bash -n`-checked as the actual multi-line script that reaches the remote shell — the same class of verification that would have caught the stdin-eating bug immediately, applied proactively this time rather than after an incident forced it.

**Rolled out to production, 2026-08-15, same day.** The manual nginx edit landed first — the live VM's `/etc/nginx/sites-available/gradeassist` turned out simpler than the repo copy implied: certbot had added `listen 443 ssl` directly onto the original block rather than duplicating it, so only one `proxy_pass` needed the `upstream ispum_api` swap, not two. `nginx -t && systemctl reload nginx` came back clean; `curl https://ispum.ru/api/health` confirmed traffic still flowed before touching the app. Then `./deploy.sh` ran its rolling restart for the first time in a genuinely new configuration — `api2` didn't exist yet, so this was creation, not recreation. Output matched the intended sequence exactly: `api2` created and confirmed healthy first, then `api` recreated and confirmed healthy, both image-matched, `[8/8]` passing both `:3000` and `:3001` directly plus the public URL. Independently re-verified after the fact rather than trusting the script's own report: `docker ps` on the VM showed both containers `Up ... (healthy)` on the new image tag, and `pm2 list` showed no app process at all (only the unrelated `pm2-logrotate` module) — ruling out the exact PM2-port-conflict failure mode from earlier in the day. Two real backend replicas, load-balanced by `least_conn`, zero-downtime rolling deploys — live.
| 1.5 ◆ | **Image scanning + SBOM** in CI (Trivy) | §15.5. Trivial once 1.1+1.3 exist; a procurement gate later. **✅ shipped 2026-08-15.** Scans the locally-loaded image on every push and PR (CRITICAL+HIGH, table format, non-blocking for now — `exit-code: '0'`), generates a CycloneDX SBOM, uploads both as a 90-day workflow artifact and prints a summary. Deliberately non-blocking on day one — see TODO.md #15 for the gated follow-up (triage the current baseline, then fail CI on new CRITICAL findings). |
...
**✅ Activated live in production, 2026-08-15, same day.** Keypair generated (`generateControlPlaneKeypair.ts`), private key added to the VM's `.env`, deployed (`efdbdd5` — migration 113 applied, seeding the `ispum-cloud` row), public key stored on that row. First tick after the deploy legitimately failed (fired before the public key was set — a real, expected sequencing gap given the row doesn't exist until the migration that runs *during* the same deploy that starts the agent) and, correctly per the lease design, held the `control_plane_heartbeat` lease for its full 10 minutes even on failure — confirmed directly via `scheduler_leases`, not assumed. A second restart after that lease naturally expired produced a fully verified live heartbeat: `deployment_heartbeats` shows `app_version = 1.5.0 (2026-08-15+efdbdd5)` matching the deployed commit exactly, and the `deployments` row's `first_seen_at`/`last_heartbeat_at`/`current_version` all populated correctly. The full sign → POST → verify → store path is confirmed working end to end against real production infrastructure, not just against tests.
| 1.7 ◆ | **`deployment_id` dimension** across admin queries + fleet overview page | §5.4. The bulk of the dashboard work, done while there is one deployment to be wrong about. **✅ done, 2026-08-15** — scope revised from the original §5.4 mapping; see below. |

**What shipped:** `listDeploymentsSummary()` (`db/queries/controlPlane.ts`) — one row per deployment, lateral-joined against its most recent heartbeat (extracted straight from the envelope JSONB, no second round trip) plus a 24h error count from `deployment_incidents`. `GET /api/admin/deployments` (existing `routes/admin.ts`, same `requireAdmin` platform-admin gate as every other admin route — no new auth surface). New `AdminDeployments.tsx` page — one card per deployment: connectivity dot (live/stale/offline per §5.5, computed client-side from `last_heartbeat_at` against a 45-minute threshold — 3× the agent's 15-min tick, enough slack for one missed tick without false-flagging), version, active seats, DB health, queue depth, 24h error count, and a relative "last signal" freshness line — the rule from §5.5 that a stale number must never render as if live is enforced by construction: every figure is paired with the freshness line it belongs to, not shown in isolation. Polls every 60s, since this page *is* the freshness signal and must not itself go stale silently. Nav entry + route added, matching every existing admin page's exact structure and conventions.

**Original §5.4 mapping revised, not just deferred — two separate decisions, made explicitly rather than by drift:**

1. **`AdminOverview` → fleet cards: dropped, not deferred.** The plan's original idea was for `AdminOverview` itself to become the one-card-per-deployment view. By the time this was reached, `Развёртывания` already existed and already filled that role — turning `AdminOverview` into a second copy of the same cards would be pure redundancy, and would also delete teacher/cost/quality stats that are daily-use platform health with nothing to do with fleet size. `AdminOverview` stays exactly as it is.
2. **`AdminUsage` / `AdminCapacity` / `AdminErrors` reworks: genuinely deferred, gated on a real second deployment.** With exactly one deployment in existence, "split by mode," "filter by deployment," and "group by deployment" all collapse to a single row/group — there's nothing for the UI to act on yet, and building `AdminCapacity`'s on-prem GPU/token metrics now means guessing at their shape before Track 0's technical spike has measured a real self-hosted deployment. Tracked as **TODO.md #14** with the exact gate (a second real deployment — cloud-pilot dedicated tenant or the on-prem deal landing, whichever comes first) and a data-source correction worth remembering when it's picked up: `AdminErrors` currently reads AI-provider call failures (`api_usage_log`); `deployment_incidents` is a *different* concept (uncaught 500s / DB-unavailable) and belongs as a new section, not a replacement of the view that's already there.

**Verification note:** typecheck (both workspaces) and a full production frontend build are clean; a live in-browser click-through was attempted but blocked by a browser-automation tooling issue in this session unrelated to the code (navigation was denied across two tabs and both protocols against an otherwise-confirmed-running dev server) — not yet clicked through in a real browser against a live login.
| 1.8 ◆ | **Expand/contract migrations adopted as policy** | §7.7. Cheapest to adopt now. **✅ shipped 2026-08-15** — CLAUDE.md Rule #12, mirrored in CONVENTIONS.md with Why/How-to-apply, and folded into the existing "Adding a migration" recipe. Policy/documentation only, no code — enforcement is "read the rule," same mechanism as every other Non-Negotiable Rule in this codebase. |
...
| 2.2 | **Runtime frontend config** — drop build-time `VITE_API_BASE_URL` | §2. **✅ shipped 2026-08-17** — see below. |
| 2.3 ▲◆ | **Prompts → content packs** (prompts, rubric templates, criteria, FGOS data), versioned and shipped independently | §15.2. **Earlier than instinct suggests** — retrofitting after go-live means three frozen months. Benefits cloud too. **Sized, not started** — prompts are inline across 20+ files in `services/`; genuinely needs its own design pass, not a same-session slice alongside smaller items. |
...
| 2.6 | **Egress policy** — no cross-provider fallback in `onprem`; fail loud | §3.5. Audit-critical. **✅ shipped 2026-08-17** — see below. |
...
| 2.9 | **`platform_admin` gating** in `onprem` mode | §15.6. **Blocked, not skipped** — §15.6 itself calls this an unresolved governance question ("Decide before go-live") and the likely answer depends on 2.7's licence file, which doesn't exist yet. Building gating logic around an undecided design would mean guessing at the wrong shape twice. |
...
**2.6 — what shipped:** `fallbackOrThrow()` (`backend/src/services/llm/registry.ts`) now checks `config.deploymentMode` before its existing silent-retry-on-DeepSeek behaviour: in `onprem`, any non-DeepSeek provider failure throws immediately instead of retrying, for every provider including a failing DeepSeek itself (which already had no further ladder to fall back to). `saas`/`dedicated` keep the exact existing behaviour — this is additive, gated on a mode nothing sets today. Matches §3.5's literal instruction ("hard-deny non-allowlisted egress and fail loudly instead of falling back") rather than trying to solve the adjacent, harder question of provider allowlisting more broadly (which providers an on-prem deployment even has configured is a 2.4/2.5 question, needing their actual answers). 4 unit tests covering saas-falls-back, onprem-throws, onprem-DeepSeek-itself-throws, and dedicated-keeps-existing-behaviour, via mocked provider classes.

**2.2 — what shipped:** a new `frontend/public/config.js` — a plain classic `<script>` loaded in `index.html` *before* the module bundle, so `window.__ISPUM_CONFIG__` is guaranteed set before any API client module evaluates. `frontend/src/lib/runtimeConfig.ts`'s `getApiBaseUrl()` reads it with a three-step precedence: a real (non-empty) runtime value wins; an unedited default (`apiBaseUrl: ""`) falls through to the existing build-time `VITE_API_BASE_URL` (kept as the local-dev fallback, since dev serves no meaningful runtime config and the Vite server/backend run on different ports); absent either, same-origin (`''`) — our own cloud's actual production behaviour today, unchanged. All four call sites that read `import.meta.env.VITE_API_BASE_URL` directly (`api/client.ts`, `api/liveJoin.ts`, `api/publicWrite.ts`, `pages/Login.tsx`) now go through this one function. The result: the same built frontend artifact can point at a different backend origin per deployment by editing one static file on the deployed host, no rebuild — confirmed `config.js` lands unmodified in `dist/` (Vite copies `public/` verbatim) and loads before the bundle, live in-browser. 4 unit tests covering all three precedence branches.
