    ...
    <div className="min-h-screen bg-bg text-ink selection:bg-amber-light selection:text-ink font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-[800px] mx-auto w-full px-6 py-16 md:py-24">
        ...
        <div className="space-y-4">
          <FAQItem 
            question="Обучается ли ИСПУМ на работах моих студентов?"
            answer="Нет. Мы категорически против использования студенческих данных для тренировки публичных языковых моделей. Загружаемые вами работы используются только локально в рамках вашего аккаунта для генерации оценки и не передаются третьим лицам для дообучения базовых моделей."
          />
          <FAQItem 
            question="Может ли система определить текст, написанный ChatGPT?" 
            answer="В настоящее время ИСПУМ фокусируется на проверке логики, аргументации и стиля по заданным вами критериям. Встроенного детектора автоматически сгенерированного текста нет, так как существующие на рынке детекторы часто дают ложноположительные результаты. Мы рекомендуем использовать ИСПУМ для глубокого анализа структуры работы."
          />
          <FAQItem 
          ...
          <FAQItem 
            ...
            answer="RAG (Retrieval-Augmented Generation) — это подход, при котором нейросеть перед ответом обращается к вашей личной базе знаний (вашим прошлым оценкам и рубрикам). Это предотвращает 'галлюцинации' и заставляет систему оценивать студентов именно в вашем авторском стиле, а не по усредненным шаблонам."
          />
          <FAQItem 
