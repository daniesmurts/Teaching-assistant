...
**Live QR quiz («Запустить в аудитории»)** 🚧 — run any generated test live in the lecture hall: a chrome-free projector screen (`/live/host/:id`) shows a join code + QR and, per question, a live answer histogram, teacher-paced through show-answer/next-question; students join from their own phones with **no account** at `/live/:code`, answering with a one-time server-issued token (answer-once enforced server-side, not just in the UI). No AI calls — pure live delivery of an already-generated test. Free: 1 session/mo, Pro/Institution: unlimited
