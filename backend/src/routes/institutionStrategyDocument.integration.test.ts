...
describe('institution strategy document (Feature Z Plane-2)', () => {
  it('GET returns null when nothing has been uploaded', async () => {
    ...
    const res = await request(app).get('/api/institution/strategy-document').set('Authorization', `Bearer ${token}`)
  ...
  it('POST uploads a document and GET reflects it', async () => {
    ...
    const get = await request(app).get('/api/institution/strategy-document').set('Authorization', `Bearer ${token}`)
  ...
  it('a second upload replaces the first (one document per institution)', async () => {
    ...
    await request(app).post('/api/institution/strategy-document').set('Authorization', `Bearer ${token}`)
  ...
  it('DELETE removes the document', async () => {
    ...
    const del = await request(app).delete('/api/institution/strategy-document').set('Authorization', `Bearer ${token}`)
