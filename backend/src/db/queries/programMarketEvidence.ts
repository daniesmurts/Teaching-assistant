...
import type { ProfstandardRef } from '../../services/marketEvidenceGenerator'
...
export async function createMarketEvidence(params: {
  ...
  const { rows } = await pool.query<ProgramMarketEvidenceRow>(
    `INSERT INTO program_market_evidence
       (program_id, region_codes, region_names, professions, vacancy_snapshot, profstandard_refs, section_text, created_by)
    ...
    [
      ...
      JSON.stringify(params.profstandardRefs), params.sectionText, params.createdBy,
    ]
  )
  return rows[0]
}
